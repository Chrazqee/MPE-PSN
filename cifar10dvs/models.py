...
# ======================================================================================================================
class SplitSpikeNeuron(nn.Module):
    def __init__(self,
                 C,
                 T: int = None,
                 tau: float = 0.25,
                 threshold: float = None,
                 surrogate_function=ZIF):
        super().__init__()
        ...
        # self.mem_proj = SeqToANNContainer(nn.Conv2d(in_channels=C, out_channels=C, kernel_size=1, stride=1, padding=0),
    ...
    def forward(self, x: torch.Tensor):
        ...
        mem_hat = torch.cat((mem_hat[:, 1:, ...], v_0), dim=1)
...
class VGGSSN(nn.Module):
    def __init__(self, tau=0.25, T=None):
        super(VGGSSN, self).__init__()
        self.tau = tau
        pool = SeqToANNContainer(nn.AvgPool2d(2))
        # pool = APLayer(2)
        self.features = nn.Sequential(
            Layer(2, 64, 3, 1, 1),
            SplitSpikeNeuron(64, T=T),  # surrogate.ATan() 做线性层的初始化权重，过线性层，再激活
            Layer(64, 128, 3, 1, 1),
            SplitSpikeNeuron(128, T=T),
            pool,
            Layer(128, 256, 3, 1, 1),
            SplitSpikeNeuron(256, T=T),
            Layer(256, 256, 3, 1, 1),
            SplitSpikeNeuron(256, T=T),
            pool,
            Layer(256, 512, 3, 1, 1),
            SplitSpikeNeuron(512, T=T),
            Layer(512, 512, 3, 1, 1),
            SplitSpikeNeuron(512, T=T),
            pool,
            Layer(512, 512, 3, 1, 1),
