...
from data.utils.box_ops import box_xyxy_to_cxcywh, box_cxcywh_to_xyxy, box_xywh_to_xyxy
...
def crop(image, target, region):
    ...
    target["size"] = torch.tensor([h, w])  # 增加了一个 'size' key-value pair
    ...
    if "boxes" in target:
        ...
        # 这个 keep keep 的好啊，过滤掉了不符合的 bbox，然后给其他参数也给 过滤一哈，这个写法学习学习！！！最后的效果就是那个框被删除了
        # 使用的是 torch 的 mask 操作
        # cropped_boxes.shape=(N, 2, 2)
...
# Normalize 是最后做的，所以在这里判断框是否是有效框应该是没啥问题的
        ...
        if "boxes" in target:
            ...
            for k in target:
                if k != 'boxes':
                    keys.append(k)
            boxes = target["boxes"]
            # 判断左上角和右下角的框的数值大小是否正确
            boxes = box_xywh_to_xyxy(boxes)
            if not (boxes[:, 2:] > boxes[:, :2]).all():  # 并不是所有的后两个数大于前两个数
                mask = (boxes[:, 2:] <= boxes[:, :2]).any(dim=-1)  # 相等也不行，因为相等的话要么是一条线，要么是一个点
                boxes = boxes[~mask, :]  # 是 True 的位置要给 mask 掉
                target['boxes'] = boxes
                for k in keys:
                    true_value = target[k][~mask]
                    target[k] = true_value

            boxes = box_xyxy_to_cxcywh(boxes)  # dataset 端使用的变换函数是 box_xywh_to_xyxy，与这里衔接上了
            boxes = boxes / torch.tensor([w, h, w, h], dtype=torch.float32)  # 进行归一化处理
            # 判断框的四个数值中是否有 nan
            target['boxes'] = boxes  # 先进行赋值操作，如果下面的条件不成立，boxes 就没有进行归一化的操作了
            if torch.isnan(boxes).any():  # 存在 是 nan 的框
                nan_mask = torch.isnan(boxes).any(dim=-1)  # 最后一个维度 有 nan 的框 即为 True
...
class Compose(object):
    def __init__(self, transforms):
        self.transforms = transforms  # a list
