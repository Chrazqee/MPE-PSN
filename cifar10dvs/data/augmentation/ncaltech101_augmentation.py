...
class FixedResolutionPad(object):
    ...
    def __call__(self, img, target):
        ...
        scale_width = self.max_pad / W
        scale_height = self.max_pad / H
        scale = min(scale_height, scale_width)

        img = F.resize(img, [int(H * scale), int(W * scale)])
        C, H, W = img.shape

        pad_x = self.max_pad - W
        pad_y = self.max_pad - H
...
if __name__ == "__main__":
    transforms_ = [RandomCrop(24), RandomHorizontalFlip(), RandomErasing()]
    compose = Compose(transforms_)
    print(compose)
