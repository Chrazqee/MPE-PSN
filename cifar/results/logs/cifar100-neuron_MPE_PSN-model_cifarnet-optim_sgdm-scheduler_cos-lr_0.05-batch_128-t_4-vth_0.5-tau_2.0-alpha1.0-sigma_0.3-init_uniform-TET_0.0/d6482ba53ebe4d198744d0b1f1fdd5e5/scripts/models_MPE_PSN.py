...
#         self.coef = nn.Parameter(torch.tensor([1 / (T - 1) for _ in range(T - 1)]).view(1, T - 1, 1, 1, 1))
...
#         # x: [N, T, ...]
#         device = x.device
#         # soft_max_x = self.soft_max(x.view(-1, *list(x.shape[2:]))).view(*list(x.shape))
...
#         # mem_hat = torch.cat((v_0, x[:, 1:, ...]), dim=1)
#
...
# todo: 静态数据集直接叠起来计算softmax导致概率都是一样的，不符合直觉；换一种度量方式
class NeuronV2(nn.Module):
    def __init__(self,
                 T: int = None,
                 tau: float = 0.25,
                 threshold: float = None,
