...
from crits import *
from dataset_utils import prepare_cifar10, prepare_cifar100
from models import *
...
parser.add_argument('--dataset', default='cifar10', type=str, help='arch')
...
if args.dataset == "cifar10":
    ...
    trainloader, testloader = prepare_cifar100(args)
...
def accumulate_inputs(inputs, T):
    inputs = inputs.unsqueeze_(dim=1)
    acc = inputs.clone()
    outputs = inputs.clone()
    for _ in range(T - 1):
        acc = torch.add(acc, inputs)
        outputs = torch.cat([outputs, acc], dim=1)
    return outputs
...
def train(epoch, writer, scheduler, args):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if args.dataset == "cifar100":
            ...
            inputs = accumulate_inputs(inputs, args.timestep)
        out_ = net(inputs)
        outputs = out_.mean(1)
        
        if args.reg:  # explicit regularize
            if args.reg == 'gram':
            ...
            if args.tet:  # use tet loss 
                ...
                loss /= net.T
                # if math.isnan(loss):
                #     raise "loss should not be zero!"
                # mem_loss = 0
                # if args.neuron == "MPE_PSN":
                #     for m in net.modules():
                #         if hasattr(m, "mem_loss"):
                #             mem_loss += m.mem_loss
                # assert mem_loss != 0
                # loss = (1 - 0.001) * loss + 0.001 * mem_loss
            loss_ = loss
...
if __name__ == '__main__':
    if not args.resume:
        uid = uuid.uuid4().hex
        path = os.path.join(
            './results/logs/cifar100-neuron_{}-model_{}-optim_{}-scheduler_{}-lr_{}-batch_{}-t_{}-vth_{}-tau_{}-alpha{}-sigma_{}-init_{}-TET_{}_NEW'.format(
                str(args.neuron),
                str(args.model),
                str(args.optim),
                str(args.scheduler),
    ...
    # 写 resume 部分的逻辑
    if args.resume:
        loguru.logger.info('load resume')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if save_max:
            torch.save(checkpoint, os.path.join(path, 'checkpoint_max.pth'))
        torch.save(checkpoint, os.path.join(path, 'checkpoint_latest.pth'))
